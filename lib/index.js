...
export default class BalanceShowHost extends Service {
	 ...
	 * probing several launchers in order: PATH pnpm, corepack pnpm, npx --yes
	 * pnpm, and the local dsh-pnpm-bin shim. Returns the captured output; a
	 * nonzero exit is reported as ok:false with the tail of the log. After a
	 * successful update the harness needs a restart to load the new bundle —
	 * the caller is told.
	 */
	async handleUpdateRun(req, res) {
		...
		if (chosen === null) {
			const payload = {
				ok: false,
				message: "pnpm not found — install pnpm (or enable corepack) to update plugins"
			};
