...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BalanceShowCard(props) {
			...
			// 持久化到 localStorage。校验合法性，脏数据回退默认。
			const [pos, setPos] = useState(() => {
				try {
					const saved = localStorage.getItem("balance-show-pos");
					if (saved) {
						...
						localStorage.removeItem("balance-show-pos");
					}
				} catch {}
				return null;
			});
			...
			const onDragStart = useCallback((e) => {
				...
				const up = () => {
					...
					try {
						...
						const ph = d?.parentH || window.innerHeight;
						const base = last ?? pos;
						localStorage.setItem("balance-show-pos", JSON.stringify({ ...base, vw: pw, vh: ph }));
					} catch {}
			...
			// ---- position safety: follow window resize by ratio, then clamp ----
			// 窗口放大/缩小时按比例跟随（保持大致相对位置），并钳制在容器内，
			// 防止缩小后卡片消失在视口外、放大后位置漂移。
			useEffect(() => {
				const followResize = () => {
					if (pos === null) return;
					...
					// 按比例跟随：新 right = 旧 right × (新容器宽 / 旧容器宽)。
					...
					const ratioRight = oldW > 0 ? pw / oldW : 1;
					const ratioBottom = oldH > 0 ? ph / oldH : 1;
					let newRight = pos.right * ratioRight;
					let newBottom = pos.bottom * ratioBottom;
					// 钳制：卡片不超出容器四边。
					const cardW = cardEl ? cardEl.offsetWidth : 280;
					const cardH = cardEl ? cardEl.offsetHeight : 56;
					const maxRight = Math.max(0, pw - cardW);
					const maxBottom = Math.max(0, ph - cardH);
					...
					if (newRight !== pos.right || newBottom !== pos.bottom) {
						...
						setPos(next);
						try {
							localStorage.setItem("balance-show-pos", JSON.stringify(next));
						} catch {}
					}
				};
				// 初始检查 + 窗口变化时检查。
									...
									updateInfo !== null && jsx("span", {
										...
										children: jsxs(Fragment, {
											children: [
												...
												jsx("span", { style: { opacity: 0.7 }, children: "npm" }),
